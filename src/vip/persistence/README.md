...
## Purpose
The persistence package provides filesystem-backed storage adapters for market data, feature matrices, and experiment artifacts.  
...
- `feature_matrix_store.py` - Parquet-based feature-matrix store keyed by symbol.
- `artifact_store.py` - JSON artifact store keyed by experiment id.
- `__init__.py` - Public persistence exports.

## Key APIs
- `ParquetMarketDataStore` - Save/load normalized OHLCV tables (`.../ohlcv.parquet`).
- `ParquetFeatureMatrixStore` - Save/load feature matrices (`.../features.parquet`).
...
2. `ParquetMarketDataStore.save(...)` persists OHLCV under `data/raw/`.
3. Feature build writes matrices with `ParquetFeatureMatrixStore.save(...)` under `data/processed/`.
4. Evaluation/reporting writes metrics with `FilesystemArtifactStore.write_json(...)`.
